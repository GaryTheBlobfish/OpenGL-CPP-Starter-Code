	...
	*/
	...
	{ 
		...
		*/
		...
		FileLogging::writeToTextFile("Game: Parsed shader res/shaders/Basic.shader successfully\n");
		...
		GLCall(glUseProgram(shader)); /* specifies to GL to use the above shader */
		FileLogging::writeToTextFile("GL: Successfully binded shader(s)\n");


		/**************** uniforms ****************/
		
		int location = glGetUniformLocation(shader, "u_Color"); /* returns location of the unif variable */
		FileLogging::writeToTextFile("GL: Accessed uniform variable u_Color\n");
		ASSERT(location != 1); /* makes sure location is not 1 */
		...
		*/
		...
		FileLogging::writeToTextFile("GL: Successfully unbound shader program\n");
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			/*
				Sets the individual components of the uniform variable 
				at the location stored in the location variable, to the
				respective parameter parsed into the function
			*/
